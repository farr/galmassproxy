...
def _plot_data(logpost, *args, **kwargs):
    pp.errorbar(np.exp(logpost.obs_masses), np.exp(logpost.obs_proxies), *args,
                fmt='.', color='k',
                xerr=(-np.exp(logpost.obs_masses)*(np.expm1(-logpost.obs_dmasses)),
                      np.exp(logpost.obs_masses)*np.expm1(logpost.obs_dmasses)),
                yerr=(-np.exp(logpost.obs_proxies)*(np.expm1(-logpost.obs_dproxies)),
...
def plot_fit_data(logpost, flatchain):
    ...
    pp.plot(np.exp(mean_masses), np.exp(proxies), '-b')
    ...
    for i in range(10):
        ...
        pp.plot(np.exp(ms), np.exp(proxies), '-b', alpha=0.25)
